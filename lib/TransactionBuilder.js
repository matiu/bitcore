...
TransactionBuilder.prototype._signScriptHash = function(walletKeyMap, input, txSigHash) {
  ...
  if (!p2sh || !p2sh.scriptType)
    throw new Error('script for input not found');
...
//
// Note that signatures are NOT verified that they are
// actually valid signatures for the transaction.
//
...
//
// Note that signatures on the given builder `inB` are NOT verified that 
// they are actually valid signatures for the transaction

TransactionBuilder.prototype.merge = function(inB) {

  // .clone will recreate the given Builder from its inputs
  // updating all
  //
  var b = inB.clone();
