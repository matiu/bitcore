...
export class ETHMULTISIGTxProvider extends ETHTxProvider {
  ...
  create(params: {
    ...
    const data = this.submitEncodeData(params);
  ...
  confirmTransactionEncodeData(params: { multisigContractAddress: string; transactionId: string }) {
  ...
  revokeConfirmationEncodeData(params: { multisigContractAddress: string; transactionId: string }) {
    const { multisigContractAddress, transactionId } = params;
    const data = this.getMultisigContract(multisigContractAddress)
